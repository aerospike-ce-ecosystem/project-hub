...
이 workflow는 [claude-code-action@v1](https://github.com/anthropics/claude-code-action)을 사용해 계획, 구현, 리뷰를 자동화합니다. aerospike-py, ACKO, cluster-manager, plugins, project-hub에서 label이 **Plan → Implement → Review** 단계를 전환합니다.
...
Issue에 `agent` label을 추가하면 planner가 실행됩니다.

Planner는 issue를 읽고 다음 작업을 수행합니다.

- 변경할 파일, 검증 방법, 영향 범위를 포함한 구현 계획을 comment로 작성합니다.
- 정보가 부족하면 `needs-clarification` label을 붙이고 필요한 내용을 질문합니다.
- 계획을 실행할 수 있는 상태가 되면 `plan-complete` label을 붙입니다.
...
Issue에 `plan-complete` label이 붙으면 구현 workflow가 시작됩니다.

구현 workflow는 marker로 구분된 plan을 읽고 다음 순서로 진행합니다.

- `in-progress` label을 붙인 뒤 코드를 수정합니다.
- build, lint, test를 실행합니다.
- Conventional Commits 형식으로 commit하고 PR을 만듭니다.
- Issue가 아니라 **PR에** `needs-review` label을 붙입니다.

### 3. PR Reviewer (`pr-reviewer.yml`)

PR에 `needs-review` label을 붙이면 reviewer의 두 job이 병렬로 실행됩니다.

**Job 1: `skill-impact-advisory`** (aerospike-py와 ACKO, Bash only)

- `gh pr diff --name-only`로 변경된 파일을 확인합니다.
- Skill에 영향을 주는 파일이 있으면 안내 comment와 `skill-impact` label을 추가합니다.

**Job 2: `review`** (Claude Code)

- PR diff에서 발견한 문제를 HIGH, MEDIUM, LOW, INFO로 분류합니다.
- HIGH와 MEDIUM 문제를 최대 5회까지 수정하고 다시 검토합니다.
- 최종 결과를 comment로 정리한 뒤 `review-complete` label을 붙입니다.
...
project-hub는 여러 repository에 걸친 작업을 조율합니다. Issue 유형과 label에 따라 아래 네 가지 workflow를 시작합니다.
...
**ADR Status Override:** 사람이 verdict label을 바꾸면 `hub-adr-status-override.yml`이 이미 merge된 ADR의 상태(`Proposed`, `Rejected`, `Deferred`)와 `검토 결과` 필드를 업데이트합니다.
...
aerospike-py나 ACKO에서 Skill과 연결된 파일이 바뀌면 Skill Impact Review가 plugins repository의 후속 작업 필요 여부를 확인합니다.
...
`start-implement` label을 붙이면 `hub-issue-dispatcher.yml`이 실행됩니다. Dispatcher는 plan comment의 HTML marker를 읽고 대상 repository마다 issue를 만듭니다.
...
Dispatcher는 잘못된 plan을 실행하지 않도록 다음 조건을 검사합니다.

- ADR issue에는 `adr-positive` label이 있어야 합니다. 없으면 dispatch를 중단하고 오류 comment를 남깁니다.
- `<!-- agent-plan-start -->` marker가 없으면 dispatch하지 않습니다.
- `<!-- repo-list: ... -->` marker가 없으면 dispatch하지 않습니다.
- Dispatch를 마치면 `dispatched` label과 생성된 issue link를 추가합니다.
...
Plan/Implement/Review pipeline 외에도 release와 CI/CD를 위한 workflow가 있습니다.

### Daily Release (`daily-release.yml`)

Daily Release는 aerospike-py, ACKO, cluster-manager에서 실행됩니다.

- **스케줄**: 매일 UTC 00:00(KST 09:00)에 실행하며 `workflow_dispatch`로 수동 실행할 수도 있습니다.
- **동작**: 마지막 tag 이후 commit이 있으면 Conventional Commits를 분석하고 version을 올립니다. 이어서 release note와 GitHub Release를 만듭니다.
- **버전 규칙**: `BREAKING CHANGE`는 major, `feat`는 minor, `fix`는 patch version을 올립니다.
...
:::warning 주의사항
- `agent` label은 목표, 범위, 완료 조건이 충분히 적힌 issue에만 추가하세요.
- 복잡한 cross-repo 변경은 project-hub의 Cross-Repo Issue 또는 Epic으로 관리하세요.
- Review loop가 5회 안에 끝나지 않으면 사람이 원인을 확인해야 합니다.
- ADR Proposal에는 `adr-positive` verdict가 나온 뒤에만 `start-implement` label을 추가하세요.
:::
...
아래 GitHub Repository/Organization 설정은 모든 repository에서 공통으로 사용합니다.
