...
description: ACKO, aerospike-py, ackoctl 사용을 돕는 CE Ecosystem Plugin의 Skill 구성
sidebar_position: 4
scope: single-repo
repos: [plugins]
...
CE Ecosystem Plugin은 ACKO 배포·운영·디버깅, aerospike-py API, ackoctl CLI, bug reporting을 안내하는 9개 Skill로 구성됩니다. 별도 Agent는 제공하지 않습니다. 기존 `acko-cluster-debugger` Agent의 기능은 `acko-debugging` Skill로 통합했습니다.
...
| **Skill** | `acko-debugging` | ACKO | 6단계 진단 절차에 따른 클러스터 디버깅(기존 `acko-cluster-debugger` Agent 기능 통합) |
