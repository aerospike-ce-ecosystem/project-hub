---
title: "ADR-0052: aerospike-py batch_read 병목 프로파일링 — 3-Methodology Cross-Validation"
description: "aerospike-py의 batch_read hot path를 stage timing in-band, py-spy --native, oha sweep 세 가지 독립 측정으로 결판내고, client-side Rust 미세 최적화의 RPS 천장을 0.32%로 확정한다."
sidebar_position: 52
scope: single-repo
repos: [aerospike-py]
tags: [adr, performance, profiling, pyo3, batch-read, gil, free-threaded, py-spy, prometheus, stage-timing, no-gil, threadpool, numpy-batch, multi-worker]
last_updated: 2026-05-23
---

# ADR-0052: aerospike-py batch_read 병목 프로파일링 — 3-Methodology Cross-Validation
...
| B7 | NumPy structured array direct path (이미 구현) | DLRM +20-40% |
...
> **"plain batch_read 시나리오에서 client-side Rust 미세 최적화(B3/B4/B5/B6)의 RPS 천장은 ≤ 0.32%로 확정한다. 1.0.0 GA의 perf 작업은 코드 미세 최적화가 아닌 시나리오 변경(B7 numpy direct / B8 multi-worker / D1+H4 no-GIL + ThreadPoolExecutor)에 집중한다."**

### 측정 결과 (세 방법론의 cross-validation)
...
```
wall per call (92 ms, b=200, c=50, 3.15 GIL):
...
| 후보 | 영역 | 측정 wall% | 최대 RPS 영향 | 진행 권장 |
|---|---|---|---|---|
| B3 (bin name PyString cache) | bin_setitem | 0.092% | ≤ 0.09% | ❌ noise floor 안 |
| B4 (PyDict capacity hint) | outer_setitem + inner_alloc | 0.013% | ≤ 0.01% | ❌ noise floor 안 |
| B5 (value PyString cache) | value_to_py 안 string subset | ~0.033% | ≤ 0.03% | ❌ noise floor 안 |
| B6 (chunked yield) | wall 자체 영향 0 (single worker) | 0% | 5-15% (multi-thread only) | △ 시나리오 의존 |
| **B7 (NumPy direct path)** | dict path 대체 (이미 구현, [ADR-0047](./2026-04-07-numpy-batch-memory-model.md)) | — | **+20-40%** (DLRM) | ✅ 권장 |
...
| B9 (PyBatchReadHandle lifecycle) | tp_dealloc + drop_in_place chain | 2.5%+ | +2-5% | △ 구조적 변경 필요 |
| B10 (PyO3 boundary 호출 통합) | trampoline + AttachGuard | 44% cumulative / 작은 leaf | +0.5% | ❌ lazy as_dict pattern과 trade-off |
...
### 대안 3: B9 (PyBatchReadHandle lifecycle 최적화) 우선 머지
...
### 대안 4 (채택): 시나리오 변경(B7/B8/D1+H4)에 집중

- **설명**: client-side Rust 미세 최적화를 docs/benchmark 자료로 archive, GA scope에서 제외. `batch_read(keys, _dtype=...)` numpy path 사용 가이드, uvicorn `--workers N` 운영 가이드, no-GIL + ThreadPoolExecutor 패턴 문서를 작성.
- **장점**:
  - 측정 RPS 효과 결정적 (+56.7% / +20-40% / +200-400%).
  - 코드 변경 최소 (B7은 이미 구현, B8/D1+H4는 문서/운영 가이드).
  - 1.0.0 GA SemVer 부담 없음.
- **단점**:
  - 진짜 Rust 내부 최적화 추진력 떨어짐 — B9/B10은 1.0.x patch cycle로 미룸.
- **선택 사유**: 측정 데이터가 결정적. 노이즈 floor (±2-5%) 안에 묻히는 최적화를 GA 전에 머지하는 것은 코드 복잡도만 증가시킴.

## 결과 (Consequences)

### 긍정적 결과

1. **결정 근거의 결정적 확보** — 3가지 독립 측정 방법론이 batch_to_dict_py wall% (0.24-0.32%) 에서 수렴. 향후 "Rust 최적화 더 할 게 있나?" 질문에 명확한 천장 제시.
2. **stage_timing 인프라 production 사용 가능** — `AEROSPIKE_PY_INTERNAL_METRICS=1` + Prometheus `/metrics` 가 이미 동작. 사용자가 자기 워크로드에서 hot path 실시간 모니터링 가능. sampling profiler 의존 X.
3. **B7 NumPy path의 진짜 가치 확정** — [ADR-0047](./2026-04-07-numpy-batch-memory-model.md)에서 결정된 Owned Arrays 메모리 모델이 DLRM 시나리오에서 +20-40% RPS 잠재력을 가진다는 측정 근거 확보. 후속 cycle에서 `/batch_read_numpy` endpoint 측정으로 검증.
4. **PyO3 free-threaded hidden cost 정량화 (업계 first)** — `PyDict::set_item` +28.7% 측정값은 PyO3 0.28 의 critical section locking 비용을 처음 정량화한 것. upstream PyO3 / CPython 3.14t/3.15t 채택 가이드 작성에 기여.
5. **macOS arm64 profiling 한계 정확히 문서화** — Tachyon sudo + py-spy --native 미지원 + qemu 우회 불가능 — 향후 측정 cycle은 arm64 Linux container를 default로.

### 부정적 결과 / 리스크

1. **client-side Rust 최적화 추진력 감소** — B3/B4/B5/B6 모두 1.0.0 GA scope에서 제외 → 일부 contributor 의 "내 PR이 noise floor 안이라 안 머지된다" 좌절감 가능. mitigations: ADR로 측정 근거 공개 + benchmark 재현 절차 문서화.
2. **B9 (PyBatchReadHandle lifecycle) 가 1.0.x 미룸으로 미해결** — 잠재 +2-5% RPS가 GA에 못 들어감. 다음 ADR cycle에서 별도 검토.
3. **debug=2 빌드 size 증가 (52MB)** — `[profile.release] debug = 1 → debug = 2 + strip = "none"` 변경은 측정용. release wheel은 default `debug = 1` 유지 (5% size 증가만 허용). 측정용 빌드는 별도 profile 또는 build flag로 분리 권장.
4. **arm64 container path가 sweep 인프라 추가** — 1.0.x 측정 cycle에 arm64 ubuntu container setup 필요. 비용은 1회성.

### 운영 가이드 (1.0.0 GA 동반 문서)

- **DLRM / data science 워크로드**: `batch_read(keys, _dtype=...)` numpy path 사용. dict path 대비 +20-40% RPS.
...
3. `oha -z 30s -c 50 'http://127.0.0.1:8200/batch_read?n=200'`
4. `curl http://127.0.0.1:8200/metrics | grep as_dict_`
5. arm64 container path: `podman run -d --platform=linux/arm64 --privileged --cap-add SYS_PTRACE ubuntu:24.04` 후 `py-spy record --native`
...
- ADR-0047 (NumPy Batch Owned Arrays): B7 path의 메모리 모델 근거
