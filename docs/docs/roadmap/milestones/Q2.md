...
:::warning 이 회고에 적힌 자동화 성과 일부는 2026-08-18에 되돌려졌습니다

아래 "에코시스템 자동화" 절과 성과 3번이 언급하는 label 트리거 에이전트 자동화(`agent-implement`, `pr-reviewer`, `issue-planner`, `hub-issue-planner`, `hub-issue-dispatcher`)는 Q3에 **보안상의 이유로 전면 제거**되었습니다. 회고는 당시 기록이므로 본문은 그대로 두고, 현재 상태만 여기에 표시합니다.

| Q2에 표준화한 항목 | 현재 |
|---|---|
| ADR Lifecycle 자동화 (ADR-0049) | Stage 1·2 제거, Stage 3(`hub-adr-status-override.yml`)만 유지 |
| CI 워크플로우 스위트 표준화 (ADR-0043) | 필수 세트에서 agent-implement / pr-reviewer / issue-planner 제외 |
| 일일 자동 릴리스 (ADR-0051) | 그대로 유지 |
| 의존성 체인 및 Merge 순서 (ADR-0050) | 원칙은 유지, 집행은 기여자 가이드로 |
| Skill Impact Review (ADR-0039) | 감지·알림은 유지, 자동 dispatch는 제거 |

제거 사유는 plan을 issue 코멘트에서 작성자 검사 없이 읽어 조직 전역 PAT로 실행하는 구조였기 때문입니다. 자세한 내용은 [ADR-0053](../../architecture/adr/2026-08-18-remove-label-triggered-agent-automation.md)과 [project-hub#158](https://github.com/aerospike-ce-ecosystem/project-hub/issues/158)에 있습니다.
:::
